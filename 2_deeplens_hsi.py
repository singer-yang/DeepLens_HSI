...
# torchrun --nproc_per_node=4 2_deeplens_hsi.py
...
from hsi_dataset import CaveDataset, KaistDataset


def config():
    """Load and prepare configuration."""
    # Load config files
    with open("configs/2_deeplens_hsi.yml") as f:
    ...
    with open(f"{result_dir}/config.yml", "w") as f:
        yaml.dump(args, f)
    shutil.copy("2_deeplens_hsi.py", f"{result_dir}/2_deeplens_hsi.py")
...
class Trainer:
    ...
    def train_epoch(self, epoch):
        ...
        if self.rank == 0:
            ...
            print(f"Train Loss: {loss_dict['total_loss'] / len(self.train_loader):.4f}")
