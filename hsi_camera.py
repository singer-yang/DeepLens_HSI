...
class HSICamera(Renderer):
    def __init__(self, lens_path, sensor_path):
        super().__init__()
        
        # RGB sensor
        sensor_config = json.load(open(sensor_path))
        self.sensor = RGBSensor.from_config(sensor_config).to(self.device)
        
        # Single DOE lens
        self.lens = DiffractiveLens(filename=lens_path, device=self.device)
    ...
    def vis_psf(self, wvln_spectral, psf_ks=201, depth=float('inf')):
        ...
        for i, wvln in enumerate(wvln_spectral):
            ...
            custom_cmap = mcolors.LinearSegmentedColormap.from_list(f'custom_{i}', colors)
        ...
        cb = mcolors.ColorbarBase(plt.gca(), cmap=plt.cm.get_cmap(cmap_name), norm=norm, orientation='horizontal')
    ...
    def render_lens(self, img_spectral, wvln_spectral, psf_ks=51):
        ...
        psf_spectral = torch.zeros((num_spectral, psf_ks, psf_ks)).to(img_spectral.device)
        for i, wvln in enumerate(wvln_spectral):
            psf_chan = self.lens.psf_infinite(wvln=wvln, ks=51)
            psf_spectral[i, :, :] = psf_chan
    ...
    def render(self, data_dict):
        ...
        # Simulate sensor response and noise
        img_rgb_raw = sensor.response_curve(img_spectral_render)
        img_rgb_noise = sensor.simu_noise(img_rgb_raw, iso)
        
        return img_rgb_noise